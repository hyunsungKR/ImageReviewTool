...
from PySide6.QtWidgets import (QApplication, QMainWindow, QFileDialog, QGraphicsView, QGraphicsScene,
                               ...
                               QMenuBar, QToolBar, QWidget, QSplitter, QMenu, QLineEdit, QMessageBox)
from PySide6.QtGui import QImage, QPixmap, QPen, QAction, QColor, QKeyEvent, QWheelEvent, QCursor, QGuiApplication
from PySide6.QtCore import Qt, QDir, QModelIndex, QRectF, QDirIterator, QFileInfo, QFile, QPoint, QObject, QSortFilterProxyModel, QRegularExpression
...
class ImageViewerController(QObject):
    ...
    def on_selection_changed(self, selected, deselected):
        ...
        if indexes:
            ...
            if not self.model.file_model.isDir(index):
    ...
    def load_image(self, index: QModelIndex):
        print(f"Loading image from index: {index}")
    ...
    def move_data_file(self):
        try:
            ...
            if selected_image:
                ...
                self.view.update_tree_view(self.model.current_folder)  # Update the tree view
                next_index = self.view.select_next_image()
                if next_index.isValid():
                    self.load_image(next_index)
        except Exception as e:
            print(f"Error moving data file: {e}")
...
class ImageViewerView(QMainWindow):
    def __init__(self, controller):
        ...
        # Right Widget for Tree View and Search Bar
        ...
        splitter.setSizes([int(self.width() * 0.8), int(self.width() * 0.2)])  # 탐색기 창의 초기 가로 값을 전체 창의 10%로 설정
    ...
    def update_tree_view(self, folder_path):
        self.tree_view.setRootIndex(self.proxy_model.mapFromSource(self.file_model.index(folder_path)))
    ...
    def display_labels(self, label_path):
        print(f"Displaying labels from: {label_path}")
        ...
        try:
            with open(label_path, 'r') as file:
                for line in file:
                    ...
                    print(f"Added label: {rect_x}, {rect_y}, {rect_width}, {rect_height}")
        except Exception as e:
    ...
    def select_next_image(self):
        ...
        if next_index.isValid():
            ...
            self.controller.load_image(self.proxy_model.mapToSource(next_index))
        return next_index
    ...
    def select_previous_image(self):
        ...
        if previous_index.isValid():
            ...
            self.controller.load_image(self.proxy_model.mapToSource(previous_index))
    ...
    def open_context_menu(self, position):
        ...
        if indexes:
            ...
            copy_file_path_action.triggered.connect(
                lambda: self.copy_to_clipboard(self.file_model.filePath(source_index)))
